...
\chapter{Komunikaty HTTP Request}
	...
	\begin{longtable}[!ht]{|p{6cm}|p{8cm}|p{6.5cm}|p{4cm}|} 
		...
		\hline	
		Adres URL & Parametry & OdpowiedŸ & Opis \\	\hline
		/api/login/ & username --- login u¿ytkownika \newline password --- has³o u¿ytkownika & "status": "ok", "token": token \tablinia "status": "ERROR PASSWORD", "token": "invalid" & Logowanie u¿ytkownika do aplikacji \\ \hline
		...
		/api/admin/certificate\_decision/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline certificate\_id --- identyfikator certyfikatu, którego dotyczy decyzja \newline decision --- decyzja True/False dotycz¹ca akceptacji certyfikatu & "status": "ok" \tablinia "status": "invalid" & Podjêcie decyzji przez administratora dotycz¹ca przyjêcia  \\ \hline
...
\chapter{Widok graficzny systemu}
	...
	Widok listy dostêpnych zamków przedstawia listê nazw zamków do jakich dany u¿ytkownik ma dostêp. U³atwieniem jest mo¿liwoœæ sortowania wyników i wyszukiwanie po nazwach. Klikniêcie w nazwê zamka powoduje otwarcie zamka. Ustawiæ mo¿na równie¿ zamek, który ma byæ otwierany automatycznie gdy jest siê w pobli¿u zamka. Kolorowe k³ódki obok nazw zamków sygnalizowaæ maj¹ status zamka. Zielona k³ódka oznacza zamek otwarty, ¿ó³ta w trakcie weryfikacji, czarna zamkniêt¹, zaœ czerwona odmowê dostêpu. (Rysunek \ref{rys:panel_listy_dostepnych_zamkow_pionowo} i \ref{rys:panel_listy_dostepnych_zamkow_poziomo})
