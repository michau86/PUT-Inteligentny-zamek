...
\begin{document}
	...
	\newpage 
	\chapter*{Wstêp}
	Inteligentny zamek powinien byæ systemem, który ma na celu zast¹pienie starego modelu zabezpieczeñ ró¿nego rodzaju drzwi i skrytek, w którym u¿ywano tradycyjnych kluczy, czy szyfrów na klucze cyfrowe, którymi bêdzie mo¿na pos³ugiwaæ siê przy pomocy smartfonów z funkcj¹ bluetooth. Celem tego rodzaju usprawnieñ bêdzie wyeliminowanie z ¿ycia codziennego sytuacji, w których u¿ytkownik musi posiadaæ pêki kluczy. Zamiast tego dziêki temu systemowi mo¿e wszystkie klucze przechowywaæ w jednym miejscu (smartfonie). System jest dedykowany przede wszystkim budynkom zawieraj¹cych wiele pomieszczeñ, które nale¿y chroniæ, lecz wykorzystany mo¿e byæ równie¿ w u¿ytku domowym.
	...
	Zadaniem urz¹dzenia steruj¹cego, w którego sk³ad wchodziæ bêd¹ Raspberry Pi 3 oraz serwomechanizm/zamek elektroniczny, jest weryfikacja klucza cyfrowego przesy³anego przez urz¹dzenie mobilne oraz otwieranie zamka przy pozytywnym wyniku weryfikacji.
	
	Oprogramowanie mikrokomputera obejmuje system Linux raspbian-jessie oraz szereg podprogramów napisanych w jêzyku Python. Skrypty programów ³¹cz¹ siê do serwera w celu pobrania informacji o poprawnoœci i dacie wa¿noœci certyfikatu dostêpu. Jeœli dane bêd¹ poprawne to zostaje wysterowany serwomechanizm (lub wys³any impuls do elektrozamka), który otwiera zamek, w przeciwnym przypadku u¿ytkownik zostanie poinformowany o odmowie dostêpu, a nieudana próba dostania siê do systemu zarejestrowana zostanie w bazie danych wraz z danymi w³aœciciela klucza.
	...
	\begin{longtable}[!ht]{|p{3.5cm}|p{7.5cm}|p{3.7cm}|} 
		...
		Funkcja & Opis & Aktorzy \\	\hline
				...
				Wys³anie ¿¹dania do serwera & Wys³anie zapytania do aplikacji serwerowej, w celu pobrania informacji & RaspberryPi, Aplikacja serwerowa\\ \hline
	...
	\begin{itemize*}
		...
		\item zainstalowany interpreter jêzyka Python 2.7 z pakietem bibliotek (RSA, Bluetooth, Lightblue, RPi.GPIO, PyCrypto, Request),
...
\newpage
	...
	Aplikacja mobilna w jêzyku Java na platformê Android ma na celu przechowywanie  w pamiêci smartfona kluczy cyfrowych u¿ytkownika oraz mo¿liwoœæ komunikacji cz³owiek-zamek-serwer. Program posiadaæ powinien interfejs graficzny, dziêki któremu bêdzie mo¿na wybraæ, który zamek chce siê otworzyæ w danej chwili. Klucz cyfrowy przesy³any bêdzie bezprzewodowo do komputera steruj¹cego zamkiem za pomoc¹ sieci bluetooth. Aplikacja powinna posiadaæ równie¿ funkcjê generowania kluczy tymczasowych, które mo¿na udostêpniaæ osobom postronnym z ustalonym okresem wa¿noœci (jednorazowy, godzinny, od poniedzia³ku do pi¹tku w godzinach od 8 do 16 itp.). W tym celu zostaje wys³ana proœba do serwera poprzez Internet o wygenerowanie klucza o okreœlonych parametrach.
	
	Klucze przesy³ane przez bluetooth powinny byæ szyfrowane algorytmem RSA (kluczem prywatnym). Klucz prywatny otrzymywany jest wraz z pierwszym certyfikatem oraz systematycznie odnawiany w celach bezpieczeñstwa. Przypadku potrzeby przeniesienia klucza dostêpowego na inne urz¹dzenie nale¿y wyeksportowaæ klucz, a nastêpnie zaimportowaæ do nowego urz¹dzenia.
	...
	\item wymagany system operacyjny Linux,
	\item zainstalowany interpreter jêzyka Python 2.7 wraz z pakietem bibliotek (MySQl, RSA, Django, PyCrypto),
	\item zainstalowany i skonfigurowany serwer Apache2, 
	...
	Diagramy poni¿ej przedstawione maj¹ mieæ na celu przybli¿enie ogólnego dzia³ania systemu. Schematy nie s¹ odzwierciedleniem poszczególnych przypadków u¿ycia, mog¹ zawieraæ nawet szcz¹tkowe odniesienia do wielu z nich. 
...
\chapter{Diagramy Klas}
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[width=25cm]{Obrazy/Diagram_klas_urzadzenie_sterujace.png}
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[width=22cm]{Obrazy/Diagram_klas_aplikacja_serwerowa.png}
		\caption{Diagram klas aplikacji serwerowej}
		\label{diagram:Diagram_klas_aplikacji_serwerowej}
	\end{figure}
	\end{landscape}
	
\chapter{Komunikaty HTTP Request}
	Modu³y systemu komunikuj¹ siê z serwerem za pomoc¹ komunikatów HTTP. Tabela \ref{tab:http_raspberry} przedstawia komunikaty serwera przeznaczone dla urz¹dzenia Raspberry~Pi. 
	
	Tabela \ref{tab:http_mobilne} zawiera komunikaty pomiêdzy serwerem, a urz¹dzeniem mobilnym.
	\begin{landscape}
	\begin{longtable}[!ht]{|p{6cm}|p{8cm}|p{6.5cm}|p{4cm}|} 
		\caption{Tabela komunikatów HTTP dla Raspberry Pi}
		\label{tab:http_raspberry}\\
		\hline	
		Adres URL & Parametry & OdpowiedŸ & Opis \\	\hline
		/api/RPI/download/cerificate/ & certificate\_id --- identyfikator certyfikatu \newline RPI\_MAC --- adres MAC urz¹dzenia \newline login\_user --- login u¿ytkownika & "data": dict\_all\_certificate, "public\_key": public\_key \tablinia "data": "invalid" & Pobranie certyfikatu u¿ytkownika \\ \hline
		/api/RPI/access\_decision/ & certificate\_id --- identyfikator certyfikatu \newline decision --- informacja o odmowie/akceptacji dostêpu & "status": "ok" \tablinia "data": "invalid" & Informacja do serwera o statusie otwierania zamka \\ \hline
	\end{longtable}
	\end{landscape}
	
	\begin{landscape}
	\begin{longtable}[!ht]{|p{6cm}|p{8cm}|p{6.5cm}|p{4cm}|} 
		\caption{Tabela komunikatów HTTP dla urz¹dzenia mobilnego}
		\label{tab:http_mobilne}\\
		\hline	
		Adres URL & Parametry & OdpowiedŸ & Opis \\	\hline
		/api/login/ & username --- login u¿ytkownika \newline password --- has³o u¿ytkownika & "status": "ok", "token": token \tablinia "status": "ERROR PASSWORD", "token": "invalid" \tablinia "status": "not activated", "token": "invalid" & Logowanie u¿ytkownika do aplikacji \\ \hline
		/api/register/ & login --- login u¿ytkownika \newline password --- has³a u¿ytkownika \newline name --- imiê u¿ytkownika \newline surname -- nazwisko u¿ytkownika \newline publickey --- klucz publiczny u¿ytkownika & "status": "ERROR" \tablinia "status": "REGISTER OK" & Rejestracja nowego u¿ytkownika do aplikacji \\ \hline
		/api/logout/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "status": "logout" \tablinia "status": "invalid" & Wylogowanie u¿ytkownika z aplikacji \\ \hline
		/api/download/all\_certifacate/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "data": dict\_all\_certificate \tablinia "status": "invalid" & Pobranie wszystkich dostêpnych certyfikatów u¿ytkownika \\ \hline
		/api/download/all\_locks/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "data": dict\_all\_locks \tablinia "status": "invalid" & Pobranie listy wszystkich dostêpnych zamków w systemie \\ \hline
		/api/download/all\_user/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "data": dict\_all\_users \tablinia "status": "invalid" & Pobranie listy wszystkich u¿ytkowników systemu \\ \hline
		/api/deactivation/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline certificate\_id --- identyfikator certyfikatu do usuniêcia & "status": "ok" \tablinia "status": "invalid" & Usuniêcie dostêpu do certyfikatu \\ \hline
		/api/request\_new\_certificate/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline lock\_id --- identyfikator zamka & "status": "ok" \tablinia "status": "invalid" & Wnioskowanie o nowy certyfikat \\ \hline
		/api/\linebreak generate\_new\_guest\_certificate/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline lock\_id --- identyfikator zamka \newline quest\_name --- imiê goœcia \newline guest\_surname --- nazwisko goœcia \newline from\_to --- data i godzina od której wazny jest certyfikat \newline period --- okres wa¿noœci certyfikatu & "status": "ok" \tablinia "status": "invalid" \tablinia "status": "denied" & Generowanie nowego certyfikatu goœcia \\ \hline
		/api/change\_password/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline newpasswd --- nowe has³o u¿ytkownika & "status": "ok" \tablinia "status": "invalid" & Zmiana has³a u¿ytkownika \\ \hline
		/api/admin/history/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "data": dict\_history \tablinia "status": "invalid" & Pobranie historii u¿ycia zamków w systemie (administrator) \\ \hline
		/api/admin/download/\linebreak all\_certificate/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "data": dict\_all\_certificate \tablinia "status": "invalid" & Pobranie wszystkich certyfikatów z systemu (administrator)\\ \hline
		/api/admin/deactivation/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline certificate\_id --- identyfikator certyfikatu do usuniêcia & "status": "ok" \tablinia "status": "invalid" & Usuniêcie dostêpu do certyfikatu (administrator) \\ \hline
		/api/admin/register\_waiting/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "status": "ok" \tablinia "status": "invalid" & Pobranie listy oczekuj¹cych u¿ytkowników na zaakceptowanie rejestracji \\ \hline
		/api/admin/register\_decision/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline user\_login --- login u¿ytkownika, którego dotyczy decyzja \newline decision --- decyzja True/False dotycz¹ca akceptacji rejestracji & "status": "ok" \tablinia "status": "invalid" & Podjêcie decyzji przez administratora dotycz¹ca rejestracji u¿ytkownika o danym loginie \\ \hline
		/api/admin/certificate\_waiting/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania & "status": "ok" \tablinia "status": "invalid" & Pobranie listy oczekuj¹cych certyfikatów na zaakceptowanie \\ \hline
		/api/admin/certificate\_decision/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline certificate\_id --- identyfikator certyfikatu, którego dotyczy decyzja & "status": "ok" \tablinia "status": "invalid" & Podjêcie decyzji przez administratora dotycz¹ca przyjêcia \\ \hline
		/api/admin/\linebreak generate\_new\_certificate/ & login --- login u¿ytkownika \newline token --- klucz sesji logowania \newline user\_id --- identyfikator u¿ytkownika \newline lock\_id --- identyfikator zamka \newline from\_date --- data od której obowi¹zuje certyfikat \newline to\_date --- data do której obowi¹zuje certyfikat \newline monday...sunday --- 7 parametrów oznaczjacych zakresy godzin w poszczególnych dniach tygodnia \newline is\_pernament --- czy dostêp jest ci¹g³y \newline name --- imiê u¿ytkownika certyfikatu \newline surname --- nazwisko u¿ytkownika certyfikatu & "status": "ok" \tablinia "status": "invalid" & Generowanie nowego certyfikatu (administrator) \\ \hline
		
	\end{longtable}
	\end{landscape}
\chapter{Projekt bazy danych}
	Baza danych bêdzie sk³adaæ siê z piêciu tabel:
	\begin{itemize*}
	...
	\item \textbf{WAIT\_LOCKS\_KEYS} --- przetrzymuje klucze dostêpowe oczekuj¹ce na zatwierdzenie przez administratora.
	\end{itemize*}
	...
	\item \textbf{TOKEN} --- generowany ci¹g pseudolosowy klucz sesji logowania,
	\item  \textbf{ISACTIVATED} --- pole boolowskie oznaczaj¹ce, czy dane konto jest zaakceptowane (aktywowane) przez administratora.
	\end{itemize*}
	...
	\item  \textbf{ADMIN\_KEY} --- wartoœæ klucza awaryjnego dla administratora.
	\end{itemize*}
\newpage
	...
	Tabela WAIT\_LOCKS\_KEYS sk³ada siê z:
	\begin{itemize*}
		\item \textbf{ID\_KEY} --- unikalny identyfikator (klucz g³ówny) oczekuj¹cego certyfikatu,
		\item \textbf{ID\_LOCK} --- klucz obcy do tabeli LOCKS, oznacza zamek do którego jest zg³aszana proœba dostêpu,
		\item \textbf{ID\_USER} --- klucz obcy do tabeli USERS, oznacza u¿ytkownika który zg³asza proœbê o dostêp do zamka.
	\end{itemize*}
	...
	\chapter{Schemat elektryczny systemu}
	Ze wzglêdu na sprzêtowy charakter projektu oraz liczne dostêpne piny GPIO urz¹dzania steruj¹cego Raspberry, nale¿y przedstawiæ schemat elektryczny systemu. Przyk³adowe pod³¹czenie zgodne z przeznaczeniem pinów przedstawiono na Rysunku \ref{schemat:schemat elektryczny systemu}.
	
	\begin{figure}[!h]
		\centering
		\includegraphics[width=14cm]{Obrazy/Schemat_elektryczny_Inteligentnego_zamka.png}
	...
	Has³a przechowywane powinny byæ w bazie danych w postaci skrótu funkcji SHA256. Ka¿de has³o sk³adaæ powinno siê z przynajmniej 6 znaków i zawieraæ co najmniej jedn¹ cyfrê.
	
	Dane u¿ytkowników znajdowaæ siê bêd¹ w bazie danych, do której jedynie bezpoœredni dostêp bêdzie z aplikacji serwera lub z konta administratora. Has³o dostêpu generowane powinny byæ za pomoc¹ generatora liczb pseudolosowych i byæ o d³ugoœci co najmniej 400bitów oraz przechodziæ podstawowe testy losowoœci.
	...
	Ka¿de po³¹czenie internetowe pomiêdzy mikrokomputerem Raspberry Pi i serwerem bêdzie zabezpieczone protoko³em IPSec. Komunikacja bluetooth nie bêdzie szyfrowana, lecz certyfikaty przesy³ane bêd¹ podpisane cyfrowo kluczem asymetrycznym RSA oraz aplikacje posiadaæ bêd¹ klucze programowe, które musz¹ byæ identyczne po jednej i po drugiej stronie po³¹czenia bluetooth. Pary kluczy (prywatny i publiczny) bêd¹ przypisane do danego u¿ytkownika.
...
\chapter{Widok graficzny systemu}
	...
	Panel rejestracji s³u¿y do utworzenie nowego u¿ytkownika poprzez podanie loginu, has³a, imienia i nazwiska u¿ytkownika oraz adresu IP serwera do którego chcemy siê zarejestrowaæ. Po upewnieniu siê, ¿e wszystkie dane s¹ poprawne, aby zakoñczyæ proces rejestracji, klikamy przycisk “ZAREJESTRUJ”. (Rysunek \ref{rys:panel_rejestracji_pionowo} i \ref{rys:panel_rejestracji_poziomo})
	...
	Widok listy dostêpnych zamków przedstawia listê nazw zamków do jakich dany u¿ytkownik ma dostêp. U³atwieniem jest mo¿liwoœæ sortowania wyników i wyszukiwanie po nazwach. Klikniêcie w nazwê zamka powoduje otwarcie zamka. Ustawiæ mo¿na równie¿ zamek, który ma byæ otwierany automatycznie gdy jest siê w pobli¿u zamka. Zmiana koloru nazw zamków sygnalizowaæ ma status zamka. Zielona napis oznacza zamek otwarty, ¿ó³ty w trakcie weryfikacji, bia³y zamkniêt¹, zaœ czerwony odmowê dostêpu. (Rysunek \ref{rys:panel_listy_dostepnych_zamkow_pionowo} i \ref{rys:panel_listy_dostepnych_zamkow_poziomo})
	...
	Panel boczny pozwala na szybkie prze³¹czanie pomiêdzy widokami. Chowany jest po lewej stronie ekranu. Umo¿liwia przechodzenie odpowiednio do listy zamków, zarz¹dzania certyfikatami, panelu administracyjnego oraz ustawieñ. Ostatnia pozycja powoduje wylogowanie z aplikacji. (Rysunek \ref{rys:panel_boczny_pionowo} i \ref{rys:panel_boczny_poziomo})
	...
	Panel listy certyfikatów, jest list¹ aktualnych certyfikatów nale¿¹cych do u¿ytkownika. Klikniêcie w dany certyfikat przenosi do widoku szczegó³owego zwi¹zanego z operacjami na tym certyfikacie. (Rysunek \ref{rys:panel_listy_certyfikatów_pionowo} i \ref{rys:panel_listy_certyfikatów_poziomo})
	...
	\begin{figure}[h]
		\begin{minipage}{0.4\textwidth}
			...
			\caption{\newline Panel wnioskowania o certyfikat (pionowo)}
...
\chapter{Mo¿liwe zagro¿enia systemu}
	G³ównym celem ataków na system Inteligentnego Zamka mo¿e byæ uzyskanie klucza prywatnego u¿ytkownika. Umo¿liwi to podszywanie siê pod inn¹ osobê, próbuj¹c otrzymaæ dostêp do nowych zamków. Ograniczeniem tej mo¿liwoœci jest decyzja administratora o przydzielenie pozwolenia oraz jeœli w³aœciciel spostrze¿e siê o naruszeniu, to mo¿e zablokowaæ konto.
	
	System nie przewiduje fizycznych zabezpieczeñ urz¹dzenia steruj¹cego, zatem maj¹c bezpoœredni kontakt z Raspberry mo¿na wówczas w ³atwy sposób wywo³aæ otwarcie zamka.
	
	Aplikacja mobilna mimo zabezpieczenia has³em oraz ponownego uwierzytelniania przy generowaniu nowych kluczy, wci¹¿ jest nara¿ona na przejêcie sesji lub urz¹dzenia u¿ytkownika.
	
	System wymaga w du¿ej mierze dostêpu do Internetu, przy braku po³¹czenia prawid³owe dzia³anie jest praktycznie niemo¿liwe. Tylko administrator jest w stanie otworzyæ zamek w takiej sytuacji.
	
\chapter{Dalsze perspektywy rozwoju projektu}
	Projekt rozwin¹æ mo¿na o zabezpieczenia wizyjne, to znaczy pod³¹czyæ kamerê komunikuj¹c¹ siê z Raspberry, która przy nieudanej próbie otwarcia zamka wykonywa³a by zdjêcie. W ten sposób ka¿da próba naruszanie bezpieczeñstwa by³aby udokumentowana. 
	
	Innym pomys³em rozwoju systemu, równie¿ z u¿yciem kamer, mo¿e byæ monitorowanie ruchu wchodz¹cego jak i wychodz¹cego na podstawie mechanizmu wykrywania twarzy. Pozwoli to zabezpieczyæ system na sytuacje, ¿e pozostan¹ w pomieszczeniu / budynku osoby,po godzinach urzêdowania. Bilansu ruchu u¿yæ mo¿na tak¿e w sytuacjach awaryjnych, kiedy wa¿na jest informacja o liczbie osób znajduj¹cych siê w pomieszczeniu.
	
	Wykrywanie twarzy mog³oby równie¿ pos³u¿yæ jako alternatywa dla po³¹czenia bluetooth. Dodaj¹c algorytmy rozpoznaj¹ce twarzy mo¿na w ten sposób decydowaæ czy dana osoba ma aktualnie dostêp czy nie.
	
	System odpowiednio zabezpieczony móg³by byæ zarz¹dzany nie tylko z poziomu aplikacji telefonu, a równie¿ z strony internetowej.
	
	Sposobem na uatrakcyjnienie systemu jest przygotowanie ró¿nych wersji urz¹dzeñ steruj¹cych w zale¿noœci od stopnia zaawansowania. Mikrokomputer Raspberry Pi jest stosunkowo drogim narzêdziem, mo¿na zast¹piæ go s³abszym kontrolerem z ograniczonymi funkcjami. Poszerza to elastycznoœæ systemu w zale¿noœci od zapotrzebowania i ceny produktu.
	
\chapter{Podzia³ prac projektowych}
	Zadania wykonane przez \textbf{Macieja Marciniak}:
	
	\begin{itemize}
		\item pe³ne oprogramowanie Raspberry Pi,
		\item aplikacja mobilna:
			\begin{itemize*}
				\item parowanie urz¹dzeñ bluetooth,
				\item transmisja danych poprzez bluetooth,
				\item uruchamianie i sprawdzanie aktywnoœci modu³u bluetooth,
				\item konsultacje projektowe widoków aplikacji,
			\end{itemize*}
		\item serwer systemu:
			\begin{itemize*}
				\item tworzenie API serwerowego,
				\item tworzenie bazy danych,
			\end{itemize*}
	\end{itemize}

	Zadania wykonane przez \textbf{Damiana Filipowicz}:
	
	\begin{itemize}
		\item aplikacja mobilna --- ca³a aplikacja, za wyj¹tkiem obs³ugi bluetooth,
		\item serwer systemu:
		\begin{itemize*}
			\item tworzenie API serwerowego,
...
\chapter{Implementacja}
	Postêpy prac oraz kod Ÿród³owy znajduje siê w repozytorium GitHub pod adresem: 
	
	{\centering\huge\textcolor{blue}{\href{https://github.com/wapet1995/PUT-Inteligentny-zamek}{Inteligentny Zamek}}}
	
\chapter{Podsumowanie projektu}
	Projekt nie zosta³ w pe³ni zrealizowany wed³ug zak³adanej dokumentacji. Du¿ym problemem okaza³o siê generowanie certyfikatu dla goœci. Tworz¹c dostêp trudno by³o utworzyæ parê kluczy prywatny --- publiczny, tak aby ka¿dy u¿ytkownik mia³ inny, a przy tym nie narusza³ bezpieczeñstwa kluczy twórcy dostêpu. Zosta³ zastosowany w takim wypadku uniwersalny klucz dla goœci, to znaczy ka¿dy z nich ma taki sam klucz. Jest to zagro¿enie dla systemu o tyle, ¿e nie jest siê w stanie potwierdziæ kto faktycznie wys³a³ dany certyfikat. Tak problem wygl¹da po stronie serwera, aplikacja mobilna ze wzglêdów czasowych nie umo¿liwia tworzenia certyfikatu dla goœcia oraz udostêpniania. Zatem w systemie nie jest mo¿liwe tworzenia dostêpu dla goœci.
	
	Nie zosta³a zrealizowana funkcja zwi¹zana z kluczem awaryjnym nale¿¹cym do administratora ze wzglêdu na wymagane przy tym zmiany w innych elementach systemu. Nie zosta³o to w pe³ni przemyœlane na odpowiednim etapie projektowania.
	
	Ze wzglêdów ekonomicznych nie zastosowano w projekcie alternatywnego zasilania UPS, lecz nie jest to czynnoœæ wymagaj¹c¹ szczególnej uwagi podczas realizacji zadania projektowego.
	
	Podsumowuj¹c ca³oœciowo projekt, system dzia³a prawid³owo, lecz nie posiada pe³nej funkcjonalnoœci zak³adaj na pocz¹tku. Podczas tworzenia projektu nauczyliœmy siê przede wszystkim dokumentowaæ system. Jest to bardzo przydatna umiejêtnoœæ, pozwalaj¹ca wykryæ b³êdy zanim przyst¹pi siê do implementacji. Dziêki pracy nad zadaniem wzmocniliœmy umiejêtnoœci zwi¹zane z prac¹ zespo³ow¹, mimo ¿e zespó³ sk³ada³ siê tylko z dwóch osób, jak równie¿ mogliœmy poszerzyæ znajomoœæ na temat pracy ze sprzêtem (tu Raspberry Pi), programowania aplikacji mobilnych oraz tworzenia systemów serwerowych.
\end{document}